...
TPA CoWork Agent ships a single binary (`tpa-cowork`) that dispatches into three modes by subcommand: desktop GUI, `tpa-cowork server`, and `tpa-cowork acp`. All three share the same release artifacts under [github.com/zzq2022/Tpa-cowork/releases](https://github.com/zzq2022/Tpa-cowork/releases) and the same Minisign-signed update manifest. The `app_update` tool is the single entry point for self-upgrade; this skill is the methodology for using it well.
...
- Use `exec` to peek at active workload: `launchctl list ai.tpacowork.server` / `systemctl --user is-active tpa-cowork.service`, plus `sessions_list({ limit: 5 })` to see if any session is mid-turn.
...
```bash
tpa-cowork --version    # should print the new version
...
| `package_manager` | brew / scoop / apt / dnf / AUR install        | Runs `brew upgrade --cask tpa-cowork` (etc.), then restarts the service.     |
...
| `service restart failed`                  | The new binary is in place but the service didn't come back. Walk the user through `launchctl kickstart -k gui/$UID/ai.tpacowork.server` / `systemctl --user restart tpa-cowork.service` manually. |
| `atomic swap … failed`                    | Suggest `app_update(action="rollback")` to restore the previous binary, then investigate before retrying. |

## Rollback

If a successful install behaves badly, the previous binary is in `~/.tpa-cowork/updater/backup/<old-version>/tpa-cowork`. Restore via:
