//! Headless `tpa-cowork` binary — the entry point shipped in the official
//! Docker image. Mirrors the `tpa-cowork server start` argv shape from
...
//!
//! Scope:
//! - `tpa-cowork server start [--bind ADDR] [--api-key KEY]` — same flags
//!   as the desktop binary; runs the HTTP/WS server and blocks until exit.
//! - `tpa-cowork knowledge-mcp` — stdio MCP wrapper for Knowledge Space.
//! - `--version` / `--help`.
//! - `tpa-cowork server {install,uninstall,status,stop,setup}` — print a
...
fn main() {
    ...
    if matches!(
        ...
        println!("tpa-cowork {}", env!("CARGO_PKG_VERSION"));
        return;
    }
    ...
    // `tpa-cowork server [sub] [opts...]`
    if args.len() >= 2 && args[1] == "server" {
        // Flag detection lives inside the `server` branch so plain
        // `tpa-cowork --help` (or anything without `server`) never prints
        ...
        match sub {
            ...
            // Sub starts with `-` → caller used `tpa-cowork server --bind …`
    ...
    if args.len() > 1 {
        eprintln!("[tpa-cowork] Unknown arguments: {:?}", &args[1..]);
        print_top_help();
        std::process::exit(1);
    }
...
/// `TPA_COWORK_SERVER_AUTO_APPROVE_TOOLS`) and emit their stderr banners. Called
...
fn apply_server_process_flags(args: &[String]) {
    ...
    if env_enabled || cli_enabled {
        ...
        // doesn't reach `~/.tpa-cowork/logs.db` — the canonical surface
...
fn print_top_help() {
    println!("TPA CoWork — headless HTTP/WebSocket server");
    println!();
    ...
    println!();
    println!("Usage:");
    println!("  tpa-cowork server start [OPTIONS]");
    println!("  tpa-cowork knowledge-mcp [OPTIONS]");
    println!("  tpa-cowork mcp [OPTIONS]");
    println!();
    ...
    println!("                                    TPA_COWORK_SERVER_AUTO_APPROVE_TOOLS=1)");
...
fn run_mcp(args: &[String]) {
    ...
    for arg in args {
        match arg.as_str() {
            ...
            "--version" => {
                println!("tpa-cowork-mcp {}", env!("CARGO_PKG_VERSION"));
                return;
            }
            "--help" | "-h" => {
                println!("TPA CoWork MCP Server (platform) — Design Space over stdio MCP.");
                println!();
                println!("Usage: tpa-cowork mcp [--allow-writes]");
                println!("  --allow-writes  Expose write tools (generate / edit / comment); default read-only");
                println!("(Knowledge Space tools remain under `tpa-cowork knowledge-mcp`.)");
                return;
            }
            other => {
...
fn parse_knowledge_mcp_args(
    ...
    for arg in args {
        match arg.as_str() {
            ...
            "--version" => {
                println!("tpa-cowork-knowledge-mcp {}", env!("CARGO_PKG_VERSION"));
                std::process::exit(0);
            }
            "--help" | "-h" => return None,
            other => {
...
fn print_knowledge_mcp_help() {
    ...
    println!();
    println!("Usage: tpa-cowork knowledge-mcp [OPTIONS]");
    println!();
    println!("Options:");
...
fn print_unsupported_subcommand(sub: &str) {
    eprintln!("`tpa-cowork server {sub}` is not supported in this build.");
    match sub {
...
fn run_server(args: &[String]) {
    ...
    let model_eval_server_token = if model_eval_mode {
        if api_key.is_some() {
            eprintln!(
                "[model-eval] --api-key is forbidden; pass TPA_COWORK_MODEL_EVAL_SERVER_TOKEN so it can be consumed before tool processes start"
            );
            std::process::exit(2);
        }
        let token = std::env::var("TPA_COWORK_MODEL_EVAL_SERVER_TOKEN")
            .unwrap_or_default()
            .trim()
            .to_string();
        std::env::remove_var("TPA_COWORK_MODEL_EVAL_SERVER_TOKEN");
        if token.len() < 24 || token.len() > 4_096 {
            eprintln!("[model-eval] TPA_COWORK_MODEL_EVAL_SERVER_TOKEN is missing or invalid");
            std::process::exit(2);
        }
        Some(token)
    } else {
    ...
    //   1. One-shot `TPA_COWORK_MODEL_EVAL_SERVER_TOKEN` in isolated evaluation mode;
    //      it has already been removed from the server environment.
    //   2. `--api-key` CLI flag (translated from `TPA_COWORK_API_KEY` env by the
    //      Docker entrypoint).
    ...
    // container without re-exporting `TPA_COWORK_API_KEY`, gets a server that
    ...
    let api_key = model_eval_server_token.or(api_key).or_else(|| {
        saved_server_config
            ...
            .inspect(|_| {
                eprintln!(
                    "[server] Using API key from saved config (server.api_key); CLI / TPA_COWORK_API_KEY would override."
                );
            })
    });
    let knowledge_agent_read_token = env::var("TPA_COWORK_KNOWLEDGE_AGENT_READ_TOKEN")
        .ok()
...
fn parse_server_args(args: &[String]) -> Option<(String, Option<String>)> {
    ...
    while i < args.len() {
        ...
        match arg {
            ...
            // dropping into `docker run ... tpa-cowork server start
