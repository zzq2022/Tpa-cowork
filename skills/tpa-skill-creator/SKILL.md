...
3. **Managed** — `~/.tpa-cowork/skills/`
4. **Project** — `.tpa-cowork/skills/` (relative to cwd, highest precedence)
...
- Determine where to save the skill:
  - **Project-level** (`.tpa-cowork/skills/<name>/`) — workflows
    specific to this repo. Ship alongside the code that depends on them.
  - **User-level** (`~/.tpa-cowork/skills/<name>/`) — cross-project
...
```bash
python skills/tpa-skill-creator/scripts/init_skill.py my-skill \
...
| Claude/TPA CoWork trigger split | + `when_to_use` |
