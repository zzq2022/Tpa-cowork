...
2. **Bootstrap by version** — a packaged install has no source tree. When the question needs implementation detail (not just runtime behavior), clone the matching release once into a reusable cache:
   ...
   - Reuse `~/.tpa-cowork/source-cache/v<version>/` if it already exists and looks complete (has `crates/`).
   - Otherwise shallow-clone that exact tag over anonymous HTTPS (never the SSH remote — packaged users can't auth it):
     ```bash
     git clone --depth 1 --branch v<version> \
       https://github.com/zzq2022/Tpa-cowork.git \
       ~/.tpa-cowork/source-cache/v<version>
     ```
...
1. Live source and docs — the working-directory checkout, or a version-matched clone under `~/.tpa-cowork/source-cache/` (see self-study to bootstrap one).
2. Local runtime data under `~/.tpa-cowork/`: start with `logs.db` and `sessions.db`, then the subsystem-specific store — `memory.db`, `knowledge/index.db`, `cron.db`, `background_jobs.db`, `local_model_jobs.db`, `recap/recap.db`, `canvas/canvas.db` — and non-DB state like `crash_journal.json` / `config.json`.
3. Settings via `get_settings`.
4. Bundled references in this skill:
   - `references/diagnostic-playbook.md`
   - `references/issue-template.md`

For SQLite diagnostics, follow the read-only rules from `tpa-logs`: use `sqlite3 -readonly` or Python URI `mode=ro`; only run `SELECT` / `.schema` / `.tables`.
